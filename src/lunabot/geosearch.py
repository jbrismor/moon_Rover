...
class GeosearchEnv(gym.Env):
    def __init__(self, render_mode=None):
        ...
        self.observation_space = spaces.Dict({
            'water_prob': spaces.Box(low=0, high=1, shape=(2, self.grid_height, self.grid_width), dtype=np.float32),
            ...
            'local_heights': spaces.Box(low=-1, high=1, shape=(5,), dtype=np.float32),
            'location_depleted': spaces.Box(low=0, high=1, shape=(1,), dtype=np.float32)
        }) # add position too for local heights?
    ...
    def _get_local_values(self, value_map, agent_pos, default=0.5):
        """
        Returns a list of local values representing relative differences 
        around the agent in the order:
        [center_diff, above_diff, below_diff, left_diff, right_diff].

        The center_diff is always 0 because it is the reference point, and
        each neighbor is given as (neighbor_value - center_value).
        """
        i, j = agent_pos
        rows, cols = value_map.shape

        center = value_map[i, j]
        above = (value_map[i - 1, j] - center) if i - 1 >= 0 else (default - center)
        below = (value_map[i + 1, j] - center) if i + 1 < rows else (default - center)
        left  = (value_map[i, j - 1] - center) if j - 1 >= 0 else (default - center)
        right = (value_map[i, j + 1] - center) if j + 1 < cols else (default - center)

        # The center difference is 0 because it is the reference
        return [0.0, above, below, left, right]
    ...
    def _update_observation(self):
        ...
        # Create the water probability channels
        water_channel = self.resource_prob_map.astype(np.float32)
        agent_pos_channel = np.zeros_like(water_channel, dtype=np.float32)
        agent_pos_channel[self.agent_pos[0], self.agent_pos[1]] = 1.0
        water_prob = np.stack([water_channel, agent_pos_channel], axis=0)  # Shape (2, H, W)
        ...
        self.obs_dict = {
            'water_prob': water_prob.astype(np.float32),
